...
# n = some other value means something is off
...
# Note- this is not a complete year of data
...
```
```{r}
LC_2025 <- read_csv(here("data/2025/LC_weather_2025.csv"))
...
LC_daily_rain <- bind_rows(daily_rain_2005_2008, daily_rain_2008_2019, daily_rain_2020, daily_rain_2021, daily_rain_2022, daily_rain_2023, daily_rain_2024, daily_rain_2025)
...
date_range <- seq(ymd('2005-01-01'),ymd('2025-12-31'), by = '1 day')

#not sure this is needed
date_range <- as_date(date_range)

#number of days there should be is 7533
length(date_range)

#number of days there are: 6800 (now 7037)
length(LC_daily_rain$date)

# number of missing days = 137 (Mostly Sep-Dec 2024)
...
cumulative_curves <-  ggplot(data = ytd,
  ...
  labs(
    ...
    subtitle = "Las Cruces 2005 to 2025",
    y = "Precipitation (mm)",
    x = "",
    caption = "2005-2025"
  ) +
  ...
  #theme_bw() +
  theme(
    #panel.background = element_rect(fill = "gray"), #solid gray background
...
ggsave("figures/combined_cumulative_2025.png", 
       plot = cumulative_curves, 
       width = 8, 
       height = 5)
```
...
annual_rain_plot <-
  ...
  scale_x_continuous(breaks =c(2005:2025)) +
...
ONI_rain_plot <-
  ...
  xlab("Year") +
  geom_text(aes(label = annual_rainfall), vjust = -0.2)+
  scale_x_continuous(breaks =c(2005:2025), expand = c(0.02,0.02)) +
  scale_y_continuous(limits = c(0,5300), expand = c(0,0)) +
  geom_hline(yintercept = median_annual, linetype = 2) +
  theme_dark() +
